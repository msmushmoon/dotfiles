...
#+begin_src bash
mkdir -p ~/Repositories/msmushmoon
cd ~/Repositories/msmushmoon
git clone https://github.com/msmushmoon/dotfiles dotfiles
cd dotfiles
./bin/config-tangle
...
#+end_src
** Env vars
Since my normal environment variables aren't set by the OS through my usual means (bash) I'm going to go ahead and grab them and set them. We do this by using a [[../shell.org::*eject environment][script I created]] that checks for differences
#+begin_src elisp
